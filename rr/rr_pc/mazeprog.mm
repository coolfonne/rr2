...
extern int up_arrow_down, down_arrow_down;
extern int strafe_right_down, strafe_left_down;
extern int fire_key_down;
...
void bricklight(void)
{
    
    glEnable(GL_LIGHTING);
    GLfloat LightAmbient[]= { 0, 0, 0, 1.0f };
    GLfloat LightDiffuse[]= { 11.04, 1.04, 11.04, 1.0f };
...
int CALLBACK WndProc (int hWnd, int message,
...
void keyboardproc(void)
{
    ...
    int dx, dy;
    TheMouse* mouse = TheMouse::getInstance();
...
void physicsloop(void)
{
    ...
    for (int ctr = 0; ctr < robots.size(); ctr++)
    {
        ...
        if (fabs(robot2.curangle - robot2.destangle) < .1) robot2.curangle=robot2.destangle;
    ...
    for (int ctr = 0; ctr < rockets.size(); ctr++)
    {
        ...
        // if (rocket.playerrocket) rocket.dy-=.0002;
    }
...
void drawgamescreen(void)
{
	...
	vector<MikePoint> theMikePoints;
	kdtree->searchtree(x-.3, y-.3, x+.3, y+.3, theMikePoints);
    
	//quads
	vector<MikePoint>::iterator i2 = theMikePoints.begin();
